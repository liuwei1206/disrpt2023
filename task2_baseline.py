...
class DetectDataset(Dataset):
    '''
    Generate the dataset for task2 connective detection
    '''
    def __init__(self, data_path, tokenizer):
        ...
        """
        self.token_list, self.label_list = self.read_detection(data_path)
    ...
    # read the data
    def read_detection(self, data_path):
        """
        Args:
...
# training
def train_detection(file_path, model_choice, tokenizer, epochs):
    #bert_model = 'bert-base-uncased'
    #tokenizer = BertTokenizer.from_pretrained(bert_model)

    dataset = DetectDataset(file_path, tokenizer)
...
def test_detection(file_path, tokenizer, model):
    dataset = DetectDataset(file_path, tokenizer)
    label2id = dataset.label2id_dic
    original_label = dataset.original_label
    loader = torch.utils.data.DataLoader(
...
def assess_detection(true_label, predict_label, label2id, original_label):
    correct = 0
    precision_list = []
    recall_list = []
    f1_list = []
